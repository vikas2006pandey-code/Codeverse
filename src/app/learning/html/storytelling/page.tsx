            ...
            <Card className="max-w-2xl text-center animate-in fade-in zoom-in-95">
                <CardHeader>
                    ...
                    <CardTitle className="text-4xl font-extrabold tracking-tighter sm:text-5xl font-headline mt-4">
                        {isVictory ? "You Saved the World!" : "Mission Failed"}
                    </CardTitle>
                    <CardDescription className="mt-4 text-lg text-muted-foreground">
                        {isVictory 
                            ? `You passed the challenge with ${character.name} and proved your skills. You are a true hero!`
...
const StorytellingCharacterSelection = ({ onSelectCharacter }: { onSelectCharacter: (character: CharacterType) => void }) => {
    // Show all characters that have a storyline with steps for HTML.
    const storyCharacters = characters.filter(c => c.storyline && c.storyline.steps && c.storyline.steps.length > 0 && c.storyline.steps.some(s => s.question.topic === 'HTML'));
        ...
        <div className="container py-12">
            ...
            <div className="text-center mb-12">
                ...
                <p className="mt-4 text-lg text-muted-foreground">
                    Select a Marvel character to guide you through an HTML learning story.
                </p>
            </div>
            ...
            {storyCharacters.length === 0 ? (
                <Card className="max-w-2xl mx-auto text-center p-8">
                    ...
                    <CardContent>
                        <p className="text-muted-foreground">It looks like there are no stories available for HTML right now. Check back later!</p>
